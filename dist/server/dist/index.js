...
// models/WithdrawalTransaction.ts
...
var insertMongoUserSchema = mongoUserSchema.omit({
  uid: true,
  createdAt: true,
...
async function registerRoutes(app2) {
  ...
  app2.use("/optimized", serveStatic("public/optimized", {
    ...
    setHeaders: (res, path2) => {
      ...
      if (path2.includes(".webp")) {
        res.setHeader("Content-Type", "image/webp");
      } else if (path2.includes(".avif")) {
...
(async () => {
  try {
    ...
    const port = process.env.PORT || (process.env.NODE_ENV === "production" ? 3001 : 5e3);
    server.listen({
      port,
      host: "0.0.0.0",
      reusePort: true
